...
Node *select(Node *T, int r)
{
  ...
  int rankVal = T->left ? T->left->size : 0;
  if(rankVal < r)
  {
      return select(T->right, r - rankVal - 1);
  }
  else if(rankVal > r)
  {
...
Node *insert_random(Node *T, int k)
{
  ...
  {
      Node *currNode = new Node(k);
      split(T, k, &currNode->left, &currNode->right);
      fix_size(currNode);
